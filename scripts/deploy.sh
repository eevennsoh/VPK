...
if atlas micros service show --service=$SERVICE_NAME --env=$ENV 2>/dev/null; then
  ...
  # For first-time deployments, verify environment variables are set
  ...
  if [ ${#MISSING_VARS[@]} -gt 0 ]; then
    ...
    echo "   ${MISSING_VARS[*]}"
    echo ""
    echo "Set them before deploying. See DEPLOYMENT_GUIDE.md Step 4:"
    echo "   atlas micros stash set -s $SERVICE_NAME -e $ENV -k <VARIABLE> -v \"<value>\""
    echo ""
    exit 1
  fi
