...
1. **Check prerequisites** → [Full checklist](../../SETUP_GUIDE.md#prerequisites--access-setup)
2. **Install dependencies** → `npm install`
3. **Generate ASAP credentials** → [ASAP Setup Guide](../../SETUP_GUIDE.md#authentication-setup-asap)
4. **Create config files** → [Configuration Guide](../../SETUP_GUIDE.md#step-3-create-envlocal-with-asap-credentials)
5. **Start servers** → `./scripts/start-dev.sh --no-wait`
...
**Note:** Deployment files (like `service-descriptor.yml`) are handled by `/deploy` command.
...
| "EADDRINUSE" error | `./scripts/stop-dev.sh` |
...
- **🔐 ASAP Authentication:** [Authentication section](../../SETUP_GUIDE.md#authentication-setup-asap)
- **🐛 Comprehensive Troubleshooting:** [Troubleshooting section](../../SETUP_GUIDE.md#troubleshooting-envlocal-issues)
- **🤖 AI Agent Instructions:** [For AI assistants](../../SETUP_GUIDE.md#-instructions-for-ai-agents)

## Next Steps

- **Develop locally:** Use `/run` to restart servers anytime
- **Ready to deploy?** Use `/deploy` to create a permanent, shareable URL
- **Make changes:** Edit code, test locally, then use `/commit` and `/deploy` again
